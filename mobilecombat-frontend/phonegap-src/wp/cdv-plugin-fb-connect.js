...
CDV.FB = {
    ...
    login: function (a, b) {
        ...
        FB.fbConnect.onConnect = function (e) {
            ...
            FB.Auth.setAuthResponse(FB.fbConnect.session, 'connected');
            //if (a) a(e);
        }
    },
    logout: function (cb) {
...
FBConnect.prototype.onLoginLocationChange = function (newLoc) {
    if (newLoc.indexOf(this.redirect_uri) == 0) {
        ...
        // TODO: Error Check
        this.session.accessToken = result.split("&")[0].split("=")[1];
        var expiresIn = parseInt(result.split("&")[1].split("=")[1]);
        this.session.expiresIn = new Date().valueOf() + expiresIn * 1000;
...
FBConnect.prototype.restoreLastSession = function () {
    var session = JSON.parse(localStorage.getItem('cdv_fb_session'));
    if (session) {
...
FBConnect.prototype.saveSession = function () {
    localStorage.setItem('cdv_fb_session', JSON.stringify(this.session));
