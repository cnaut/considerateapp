...
public class ConsiderateAppActivity extends Activity {
	...
	private final BroadcastReceiver receiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			} else if (intent.getAction().equals(Intent.ACTION_USER_PRESENT)) { // VS
				// ACTION_SCREEN_ON
				// WHAT TO DO WHEN SCREEN IS ON
				numLocks++;
				stopwatch.start();

				String phoneUnlockText = "<body bgcolor = 'black'><font color= 'white' size = 5><center>You have unlocked your phone "
						+ numLocks
	...
	class timerClearTask extends TimerTask {
		public void run() {
			...
			String plotPoints = ""
					+ Double.toString(((tMinus5 / max) * 100.00)) + ","
					+ Double.toString(((tMinus4 / max) * 100.00)) + ","
					+ Double.toString(((tMinus3 / max) * 100.00)) + ","
					+ Double.toString(((tMinus2 / max) * 100.00)) + ","
					+ Double.toString(((tMinus1 / max) * 100.00));
			graphString = "<img src='http://chart.apis.google.com/chart?chtt=Number+of+Unlocks&amp;chts=ffffff,12&amp;chs=300x150&amp;chf=bg,s,000000|c,s,000000&amp;chxt=x,y&amp;chxl=0:|T-5|T-4|T-3|T-2|T-1|1:|0|"
					+ (double) ((double) max / 2.0)
					+ "|"
					+ max
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		Intent flipIntent = new Intent(getApplicationContext(),
				FlipIntentService.class);
		startService(flipIntent);

		IntentFilter filter = new IntentFilter(Intent.ACTION_USER_PRESENT); // vs
		// ACTION_SCREEN_ON
